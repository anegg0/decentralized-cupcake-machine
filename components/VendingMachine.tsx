...
export const VendingMachine = (props: { id: string; type: string }) => {
  ...
  class Web3VendingMachineClient {
    ...
    async giveCupcakeTo(identity, vendingMachineContractAddress) {
      ...
      const cupcakeCountBefore = Number(
        await contract.getCupcakeBalanceFor(identity),
      );
      const transaction = await contract.giveCupcakeTo(identity);
      const receipt = await transaction.wait();
      const cupcakeCountAfter = Number(
        await contract.getCupcakeBalanceFor(identity),
      );
    ...
    async getCupcakeBalanceFor(identity, vendingMachineContractAddress) {
      ...
      console.log(
        `getting cupcake balance for ${identity} on ${vendingMachineContractAddress}`,
      );
      const contract = await this.initContract(vendingMachineContractAddress);
  ...
  const vendingMachineClient =
    props.type == "web2"
      ? new Web2VendingMachineClient()
      : new Web3VendingMachineClient();
  ...
  const handleCupcakePlease = useCallback(async () => {
    try {
      ...
      if (vendingMachineClient.isWeb3) {
        gotCupcake = await callWeb3VendingMachine(
          vendingMachineClient.giveCupcakeTo,
        );
      } else {
        gotCupcake = await (
          vendingMachineClient as Web2VendingMachineClient
        ).giveCupcakeTo(identity || "no name");
      }
  ...
  const handleRefreshBalance = useCallback(async () => {
    try {
      if (vendingMachineClient.isWeb3) {
        setCupcakeBalance(
          await callWeb3VendingMachine(
            vendingMachineClient.getCupcakeBalanceFor,
          ),
        );
      } else {
        setCupcakeBalance(
          await (
            vendingMachineClient as Web2VendingMachineClient
          ).getCupcakeBalanceFor(identity || "no name"),
        );
      }
      updateSuccessIndicator(true);
    } catch (err) {
  ...
  return (
    <div className="vending-machine">
      ...
      <input
        type="text"
        placeholder={
          "Enter " + vendingMachineClient.identityLabel.toLowerCase()
        }
        value={identity}
        onChange={(event) => setIdentity(event.target.value)}
      />
      <label className={vendingMachineClient.isWeb3 ? "" : "hidden"}>
        Contract address
      </label>
      <input
        type="text"
